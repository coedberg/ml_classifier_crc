...
sd_rare = amp_subset_samples(data = d, minreads = 10000, normalise = FALSE, rarefy = 10059)

subsetmd_rare <- sd_rare[["metadata"]]

qc_fail_rare <- anti_join(metadata, subsetmd_rare, copy = TRUE)

md_rare <- anti_join(metadata, qc_fail_rare) #removes the samples from the metadata
...
df_rare$dx <- as.factor(df_rare$dx)
df_rare <- df_rare[,-461]
