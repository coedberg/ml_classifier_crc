...
rarecurve<- amp_rarecurve(
  ...
  #scale_x_continuous(breaks = c(0, 10000, 20000, 30000, 40000, 50000, 60000), limits = c(0, 50000))+ 
...
ggsave(plot = rarecurve, filename = "rarecurve.jpeg",
       path = "C:/Users/Berg/Desktop/Thesis/Results/Exploration/",
...
test <- as.data.frame(sd[["metadata"]][["dx"]])
colnames(test) <- "dx"
test %>% 
  filter(dx == "normal")
...
obj = amp_alphadiv(sd, richness = T)
...
df_genera <- df %>% pivot_longer(-c(dx, seq_id), names_to="genus", values_to="rel_abund")
...
num_features <- ggplot(bar, aes(x=name, y=value, fill=name)) +
  ...
  coord_flip()+
  theme_classic()+
  scale_fill_manual(values= c( "#FF7F00","#E41A1C", "#377EB8" , "#4DAF4A"))+
  theme(legend.position = "none")+
